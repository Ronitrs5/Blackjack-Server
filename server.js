...
io.on('connection', (socket) => {
    ...
    socket.on('hit', (roomId, playerId) => {
        ...
        if(room.players[playerId].is_out){
            socket.emit('playerOutError', '');
            console.log('Player is out');
            return;
        }

        if(!room.players[playerId].is_chance){
            socket.emit('notChanceError', '');
            console.log('Not chance');
            return;
        }
        ...
        if (room && room.players[playerId]) {
            ...
            if (value === 'Jack') {
                ...
                cardValue = value; // For numbers 2-10, just use the value
            }
            ...
            console.log(room.card_deck);
            console.log(player.cards_in_hand);
            
            // Update the player's sum
            player.cards_sum = calculateCardSum(player.cards_in_hand);
            console.log(player.cards_sum);
            console.log(JSON.stringify(rooms, null, 2))

            if(player.cards_sum > 21){
                console.log(`${player.id} is busted`);
                player.is_out = true;
            }
            
            
            io.to(roomId).emit('playerHit', playerId, newCard, player.cards_sum, room.players);
    ...
    // Helper function to toggle player chance
// Helper function to toggle player chance
function toggleChance(roomId, room, currentPlayerId) {
    const playerIds = Object.keys(room.players);
    const currentIndex = playerIds.indexOf(currentPlayerId);
    const nextIndex = (currentIndex + 1) % playerIds.length; // Wrap around if at the end
    ...
    io.to(roomId).emit('updatePlayers', room.players); // Emit update with the correct roomId
}
